...
{
    protected $metaDataOrder = 111;

    protected $metaDataNameKey = 'Funnels_FunnelXFlow';
    ...
    public function configureWidgets(WidgetsList $widgetsList, ReportWidgetFactory $factory)
    {
        ...
        foreach ($funnels as $funnel) {
            ...
            $config->setCategoryId('Funnels_FunnelsReports');
            $widgetsList->addWidgetConfig($config);
        }
    }
    ...
    public function configureReportMetadata(&$availableReports, $infos)
    {
        ...
        $order = $this->metaDataOrder; // goals start at 50, we want to show them after goals
        foreach ($funnels as $funnel) {
            $order = $order + 2;

            $funnel['name'] = Common::sanitizeInputValue($funnel['name']);

            $this->name       = Piwik::translate($this->metaDataNameKey, $funnel['name']);
            $this->parameters = [

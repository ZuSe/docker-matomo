    ...
    public static function getIdByName($valueToMatch, $sqlField, $matchType, $segmentName)
    {
        ...
        switch ($matchType) {
            case SegmentExpression::MATCH_NOT_EQUAL:
                $where = ' (lg.name != ? OR (lf.idgoal = 0 AND lf.name != ?)) ';
                break;
            ...
            case SegmentExpression::MATCH_CONTAINS:
                ...
                $where = ' (lg.name LIKE CONCAT(\'%\', ?, \'%\') OR (lf.idgoal = 0 AND lf.name LIKE CONCAT(\'%\', ?, \'%\'))) ';
                break;
            case SegmentExpression::MATCH_DOES_NOT_CONTAIN:
                $where = ' (lg.name NOT LIKE CONCAT(\'%\', ?, \'%\') OR (lf.idgoal = 0 AND lf.name NOT LIKE CONCAT(\'%\', ?, \'%\'))) ';
                break;
        ...
        if ($valueToMatch === Piwik::translate('Ecommerce_Sales')) {
            $where .= ' OR (lf.idgoal = 0 AND lf.name IS NULL) ';
        }

        $sql .= $where;

        return array('SQL' => $sql, 'bind' => [$valueToMatch, $valueToMatch]);
    }
}
